...
function panel.draw(widget, x, y, battId)
  ...
  if status.battery[1+1] * 0.01 < 10 then
    lcd.drawNumber(x+68, y+-4, status.battery[1+1] + 0.5, PREC2+MIDSIZE+RIGHT+flags)
  else
    lcd.drawNumber(x+68, y+-4, (status.battery[1+1] + 0.5)*0.1, PREC1+MIDSIZE+RIGHT+flags)
  end
  ...
  if status.battery[1+2] * 0.01 < 10 then
    lcd.drawNumber(x+68, y+45, status.battery[1+2] + 0.5, PREC2+MIDSIZE+RIGHT+flags)
  else
    lcd.drawNumber(x+68, y+45, (status.battery[1+2] + 0.5)*0.1, PREC1+MIDSIZE+RIGHT+flags)
  end
  ...
  -- POWER -- (95*0.667=63, 24*0.882=21, 97*0.667=65, 31*0.882=27)
  -- power 1
  lcd.setColor(CUSTOM_COLOR,WHITE) -- white
