...
function panel.draw(widget, x, y, battId)
  ...
  -- display capacity bar % (10*0.667=7, 57*0.882=50, 100*0.667=67, 28*0.882=25)
  lcd.setColor(CUSTOM_COLOR,lcd.RGB(255,255, 255))
  lcd.drawFilledRectangle(x+7, y+50,67,25,CUSTOM_COLOR)
  if perc > 50 then
  ...
  lcd.drawGauge(x+7, y+50,67,25,perc,100,CUSTOM_COLOR)
  -- battery percentage (40*0.667=27, 53*0.882=47)
  lcd.setColor(CUSTOM_COLOR,utils.colors.black) -- black

  local strperc = string.format("%02d%%",perc)
  lcd.drawText(x+27, y+47, strperc, MIDSIZE+CUSTOM_COLOR)

  lcd.setColor(CUSTOM_COLOR,utils.colors.white) -- white
  if status.showMinMaxValues == false then
    if status.battLevel2 == false and status.alarms[8][2] > 0 then
      -- 30*0.667=20, 14*0.882=12
      utils.drawBlinkBitmap("cell_red_blink_86x30",x+20,y+12)
      utils.lcdBacklightOn()
    elseif status.battLevel2 == true then
      lcd.drawBitmap(utils.getBitmap("cell_red_86x30"),x+20,y+12)
    elseif status.battLevel1 == false and status.alarms[7][2] > 0 then
      utils.drawBlinkBitmap("cell_orange_blink_86x30",x+20,y+12)
      utils.lcdBacklightOn()
    elseif status.battLevel1 == true then
      lcd.drawBitmap(utils.getBitmap("cell_orange_86x30"),x+20,y+12)
      lcd.setColor(CUSTOM_COLOR,utils.colors.black) -- black
    end
  end
  flags = CUSTOM_COLOR
  -- (101+2)*0.667=69, 10*0.882=9
  if status.battery[1+battId] * 0.01 < 10 then
    lcd.drawNumber(x+69, y+9, status.battery[1+battId] + 0.5, PREC2+MIDSIZE+RIGHT+flags)
  else
    lcd.drawNumber(x+69, y+9, (status.battery[1+battId] + 0.5)*0.1, PREC1+MIDSIZE+RIGHT+flags)
  end

  -- 103*0.667=69, 25*0.882=22
  local lx = x+69
  lcd.drawText(lx, y+22, "V", SMLSIZE+flags)
  -- labels
  ...
  -- 110*0.667=73, 89*0.882=79
  lcd.drawText(x+73, y+79, "IMU TEMP", SMLSIZE+RIGHT+CUSTOM_COLOR)

  -- IMU Temperature (98*0.882=86)
  lcd.setColor(CUSTOM_COLOR,utils.colors.white)
  lcd.drawText(x+73, y+86, string.format("%d%s",telemetry.imuTemp, conf.degSymbol), MIDSIZE+RIGHT+CUSTOM_COLOR)

  if status.showMinMaxValues == true then
    -- (101+11)*0.667=75, (10+8)*0.882=16
    libs.drawLib.drawVArrow(x+75, y+16,false,true)
  end
