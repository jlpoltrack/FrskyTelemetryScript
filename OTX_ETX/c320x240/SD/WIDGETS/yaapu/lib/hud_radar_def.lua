...
-- viewport: 280x134 -> 160x115 (matching HUD area)
...
-- scaled viewport dimensions matching HUD area
local viewWidth = 160
local viewHeight = 115
...
function panel.draw(widget)
  ...
  if bit32.band(myCode,1) == 1 then
    ...
    local newOriginX = 80 + viewWidth/2 - (zoom*myX*0.5);
    local homeCode = libs.drawLib.computeOutCode(newOriginX, originY, minX+13, minY+13,maxX-13, maxY-13);
    if bit32.band(homeCode,2) == 2 then
      zoom = zoom * 0.95
    end
    originX = 80 + viewWidth/2 - (zoom*myX*0.5);
  end
  ...
  if bit32.band(myCode,8) == 8 then
    ...
    local newOriginY = 16 + viewHeight/2 - (zoom*myY*0.5);
    local homeCode = libs.drawLib.computeOutCode(originX, newOriginY, minX+13, minY+13,maxX-13, maxY-13);
    if bit32.band(homeCode,4) == 4 then
      zoom = zoom * 0.95
    end
    originY = 16 + viewHeight/2 - (zoom*myY*0.5);
  end
  ...
  -- home icon size: 20*0.667=13
  libs.drawLib.drawHomeIcon(originX-13/2,originY-13/2)
  ...
  lcd.drawText(minX,minY,string.format("zoom:%.02f",zoom),SMLSIZE+CUSTOM_COLOR)
  lcd.drawText(minX,minY+13,string.format("dist:%d",telemetry.homeDist),SMLSIZE+CUSTOM_COLOR)

  lcd.setColor(CUSTOM_COLOR,utils.colors.white)

  if dist(myScreenX, myScreenY, originX, originY) < viewHeight/2 then
    if zoom < 0.5 then
