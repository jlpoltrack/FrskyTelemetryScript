...
local function drawRssi()
  -- top bar layout: RS at x=100, TX voltage at x=230
  lcd.drawText(100, 0, "RS:", SMLSIZE+CUSTOM_COLOR)
  lcd.drawText(118, 0, getRSSI(), SMLSIZE+CUSTOM_COLOR)
  -- TX voltage
  local vtx = string.format("%.1fv",getValue(getFieldInfo("tx-voltage").id))
  lcd.drawText(245, 0, vtx, SMLSIZE+CUSTOM_COLOR+RIGHT)
end

local function drawRssiCRSF()
  lcd.setColor(CUSTOM_COLOR,utils.colors.white)
  -- top bar layout: RTP at x=100, TX voltage at x=230
  lcd.drawText(100, 0, "RTP:", SMLSIZE+CUSTOM_COLOR)
  lcd.drawText(122, 0, string.format("%d/%d/%d",getValue("RQly"),getValue("TQly"),getValue("TPWR")), SMLSIZE+CUSTOM_COLOR)
  -- TX voltage
  local vtx = string.format("%.1fv",getValue(getFieldInfo("tx-voltage").id))
  lcd.drawText(245, 0, vtx, SMLSIZE+CUSTOM_COLOR+RIGHT)
end
...
local function drawMessagesTelemetryBar()
  ...
  lcd.setColor(CUSTOM_COLOR,colorLabel)
  lcd.drawText(LCD_W-4, 55-3, "BATT %", SMLSIZE+CUSTOM_COLOR+RIGHT)
  lcd.setColor(CUSTOM_COLOR,WHITE)
  lcd.drawText(LCD_W-4, 55+7, strperc, MIDSIZE+CUSTOM_COLOR+RIGHT)
  ...
  lcd.setColor(CUSTOM_COLOR,colorLabel)
  lcd.drawText(LCD_W-2, 125-3, string.format("%s %s", speedLabel, conf.horSpeedLabel), SMLSIZE+CUSTOM_COLOR+RIGHT)
  lcd.setColor(CUSTOM_COLOR,WHITE)
  lcd.drawText(LCD_W-2, 125+7, string.format("%.01f",speed), MIDSIZE+CUSTOM_COLOR+RIGHT)
  ...
  lcd.setColor(CUSTOM_COLOR,colorLabel)
  lcd.drawText(LCD_W-2, 160-3, string.format("HOME %s", label), SMLSIZE+CUSTOM_COLOR+RIGHT)
  lcd.setColor(CUSTOM_COLOR,WHITE)
  lcd.drawNumber(LCD_W-2, 160+7, dist, MIDSIZE+flags+CUSTOM_COLOR+RIGHT)
  ...
  lcd.setColor(CUSTOM_COLOR,colorLabel)
  lcd.drawText(LCD_W-4, 90-3, string.format("%s %s", altLabel, unitLabel), SMLSIZE+CUSTOM_COLOR+RIGHT)
  lcd.setColor(CUSTOM_COLOR,WHITE)
  lcd.drawText(LCD_W-4, 90+7, string.format("%.0f",alt), MIDSIZE+CUSTOM_COLOR+RIGHT)
  -- home angle (y=185)
  lcd.setColor(CUSTOM_COLOR,utils.colors.darkyellow)
  libs.drawLib.drawRVehicle(LCD_W-25,185,10,math.floor(telemetry.homeAngle - telemetry.yaw),CUSTOM_COLOR)
end
...
local function drawMessageScreen()
  ...
  lcd.drawFilledRectangle(LCD_W - 50, 16, 50, LCD_H - 36, CUSTOM_COLOR)
  ...
  lcd.drawLine(0,LCD_H-20,LCD_W,LCD_H-20,SOLID,CUSTOM_COLOR)

  lcd.setColor(CUSTOM_COLOR,WHITE)
  if status.strFlightMode ~= nil then
    lcd.drawText(0,LCD_H-20,status.strFlightMode,CUSTOM_COLOR)
  end
  lcd.drawTimer(LCD_W-2, LCD_H-20, model.getTimer(2).value, CUSTOM_COLOR+RIGHT)
end
...
local function drawMsgDialog(msg)
  ...
  lcd.setColor(CUSTOM_COLOR,utils.colors.white)
  lcd.drawText(math.floor(LCD_W/2), 65 + 22, msg, DBLSIZE+CUSTOM_COLOR+CENTER)
end
...
local function drawFullScreen(widget)
  ...
  if not (resetPending or resetLayoutPending or loadConfigPending) then
    ...
    if drawStatus then
      libs.drawLib.drawFailsafe(100, 45)
      libs.drawLib.drawArmStatus(100, 45 + 40)
    end
  else
...
-- are we full screen? lower threshold for 320x240 screens
local function drawScreen(widget, event, touchState)
    if widget.zone.h < 200 then
